...
def generate_s3_url_for_current_driver(imageName):
    s3_client = get_boto3_client()
    image_key_final = 'current_driver/' + str(imageName) + '.jpeg'
    response = None
    try:
        response = s3_client.generate_presigned_post(Bucket=os.getenv('AWS_BUCKET_NAME'),
                                                     Key=image_key_final,
    ...
    return {'result': response, 'status': 200}, image_key_final
...
def get_image_url(image_key):
    ...
    try:
        response = s3_client.generate_presigned_url('get_object',
                                                    Params={'Bucket': os.getenv('AWS_BUCKET_NAME'), 'Key': image_key},
...
def get_image_url_for_current_driver(image_key):
    s3_client = get_boto3_client()
